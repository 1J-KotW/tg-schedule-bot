...
- **DB**: PostgreSQL (продакшн) + SQLite per tenant (разработка), Alembic/SQL миграции и вспомогательные скрипты
...
## 📊 Базы данных и архитектура данных

### Продакшн архитектура
- **PostgreSQL**: основная БД для высоконагруженных запросов
- **Redis**: кэширование горячих данных, сессии пользователей
- **SQLite**: локальная разработка и тестирование per tenant

### Модель данных
```sql
-- Основные таблицы
groups (id, name, faculty, course, tenant_id)
teachers (id, name, department, tenant_id)  
schedules (id, group_id, teacher_id, subject, room, datetime, tenant_id)
users (id, telegram_id, last_group_id, last_teacher_id, tenant_id)
```

### Изоляция данных
- **Multi-tenant**: `tenant_id` во всех таблицах для изоляции
- **Индексы**: оптимизированы для запросов по дате и группе
...
USE_SQLITE=true
TENANTS_DATA_PATH=./data/tenants
```
3) Запуск API
```bash
python run.py
...
- `DATABASE_URL` — PostgreSQL URL для продакшна
...
- `DB_POOL_SIZE`, `DB_MAX_OVERFLOW` — настройки пула соединений PostgreSQL

## Производительность и наблюдаемость
- **PostgreSQL**: оптимизированные запросы с индексами, connection pooling
...
- **Мониторинг**: метрики производительности БД, время ответа API
